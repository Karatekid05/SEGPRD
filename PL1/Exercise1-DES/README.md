...
On OpenSSL 3.x, DES is disabled by default. The script tries **both** providers (`-provider default -provider legacy`) automatically: **legacy** provides DES; **default** provides the random number generator (RNG). Using only `-provider legacy` can cause `RAND_bytes failed` / CTR-DRBG errors.

**Manual encrypt (example):**

```bash
openssl enc -des-cbc -provider default -provider legacy -salt -in message.txt -out message.des -k segprd123
```

**Manual decrypt:**

```bash
openssl enc -des-cbc -d -provider default -provider legacy -in message.des -out message_decrypted.txt -k segprd123
```
...
| `message.des` | (c) The encrypted payload (binary) |
| `message_decrypted.txt` | (d) Result of decrypting with the **same** key (should equal `message.txt`) |
| `message_decrypted_fail.txt` | (e) Result of decrypting with a **different** key (garbage) |
...
| `message.des` | Part 1: (c) encrypted payload |
| `message_decrypted.txt` | Part 1: (d) decryption with correct key |
| `message_decrypted_fail.txt` | Part 1: (e) decryption with wrong key (garbage) |
